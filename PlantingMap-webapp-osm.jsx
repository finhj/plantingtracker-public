...
function defaultLocations() {
  ...
  const single = (id, name, type, lat, lng) => ({ id, name, type, sections: [{ id: id + "_main", name, lat, lng, beds: 1, direction: "N", groupBreakpoints: null }] });
  const sec = (id, name, lat, lng) => ({ id, name, lat, lng, beds: 1, direction: "N", groupBreakpoints: null });
  ...
  useEffect(() => {
    ...
    allSections.forEach((s) => {
      ...
      if (!marker.getTooltip()) {
        ...
        if (editMode || s.type === "reference") marker.openTooltip(); else marker.closeTooltip();
      }
    });
  }, [leafletReady, allSections, editMode]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div style={{ background: PAPER, minHeight: "100vh", color: INK, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: 16 }}>
        ...
        />
        {!leafletReady && (
          <div style={{ fontSize: 12, color: "#8a8272", marginTop: 6 }}>Loading map…</div>
        )}
