...
- Offline support (needs a network connection to load/save)
- The farm map photo (`public/farm-map.jpg`) is the same hand-annotated one
  from the Claude version — replace it if you get a better one, just keep
  the filename or update `MAP_IMAGE` in `src/PlantingMap.jsx`.
